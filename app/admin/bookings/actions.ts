// server only — handles confirm/cancel emails from admin dashboard
"use server";
...
// escapes HTML in user data before putting it in emails
...
// strips \r\n\t from email headers — prevents header injection
function sanitizeHeader(s: string): string {
  return s.replace(/[\r\n\t]/g, " ").trim().slice(0, 200);
}

// created once at module level so it's reused across calls
...
// email layout wrapper — accentColor is the stripe (green=confirmed, red=cancelled)
...
// styled details box from array of [label, value] pairs
...
export async function sendStatusEmail(bookingId: string, status: "confirmed" | "cancelled") {
  // stops immediately if no admin session — blocks unauthenticated requests
  await requireAuth();

  // fetch from DB by ID — email content always reflects what's actually in the DB
  ...
  if (status === "confirmed") {
    ...
    // only email advisor if they have one — some members are added without email
    if (advisorEmail) {
      await transporter.sendMail({
        from: `"CAPHA" <${process.env.EMAIL_USER}>`,
        to: advisorEmail,
    ...
    // cancellation — email student first
    await transporter.sendMail({
      from: `"CAPHA" <${process.env.EMAIL_USER}>`,
      to: studentEmail,
    ...
    // also tell the advisor the session is off
    if (advisorEmail) {
      await transporter.sendMail({
        from: `"CAPHA" <${process.env.EMAIL_USER}>`,
        to: advisorEmail,
